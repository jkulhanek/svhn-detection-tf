...
def policy_vtest():
    ...
    # sub-policy that will be applied sequentially on the image.
    policy = [
        # [('TranslateX_BBox', 1.0, 1), ('Equalize', 1.0, 1)],
        [('Rotate_BBox', 1.0, 10), ('Color', 1.0, 6)],
    ]
    return policy
...
def _apply_multi_bbox_augmentation(image, bboxes, prob, aug_func,
    ...
    def wrapped_aug_func(_image, bbox, _new_bboxes): return _apply_bbox_augmentation_wrapper(
    ...
    def cond(_idx, _images_and_bboxes): return tf.less(_idx, num_bboxes)
    ...
    # bboxes in the image.
    # pylint:disable=g-long-lambda
    def body(_idx, _images_and_bboxes): return [
...
def distort_image_with_autoaugment(image, bboxes, args):
    ...
    Args:
      image: `Tensor` of shape [height, width, 3] type of uint8 representing an image.
      bboxes: `Tensor` of shape [N, 4] type of float32 representing ground truth boxes that are
        normalized between [0, 1].
    ...
    """
    logging.info('Using autoaugmention policy: %s', args.augmentation_name)
    available_policies = {'v0': policy_v0, 'v1': policy_v1, 'v2': policy_v2,
                          'v3': policy_v3, 'test': policy_vtest}
    if args.augmentation_name not in available_policies:
        raise ValueError('Invalid augmentation_name: {}'.format(args.augmentation_name))

    policy = available_policies[args.augmentation_name]()
